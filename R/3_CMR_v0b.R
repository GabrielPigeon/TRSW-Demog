...
# same a v0, but adding age-dep farm random effect and no envEffect
...
myCode <- nimbleCode({
    ...
    for (f in 1:nb.farm){
        eps.farm[f,1:9]  ~ dmnorm(zero[1:9], Tau.farm[1:9, 1:9])
        for (a in 1:3){
            s.ranef.farm[f,a] <- xi.farm[a] *eps.farm[f,a]
            r.ranef.farm[f,a] <- xi.farm[a+3] *eps.farm[f,a+3]
            f.ranef.farm[f,a] <- xi.farm[a+6] *eps.farm[f,a+6]
        }
    } #f
    Tau.farm[1:9, 1:9]  ~ dwish(W[1:9, 1:9], 10)
    Sigma.farm.raw[1:9, 1:9] <- inverse(Tau.farm[1:9, 1:9])
    for (k in 1:9){# get the posterior estimation of sigma and correlation
        for (k.prime in 1:9){
...
myInits <- function(curDat,curConst){
    l=list(
        ...
        Tau.farm=inverse(diag(rep(c(0.2,0.5,0.1),each=3))),
        Tau.id=inverse(diag(c(1.5,0.1))),
        sig=rnorm(1,3,.5),
        xi.farm=runif(9,0.95,1.05),
